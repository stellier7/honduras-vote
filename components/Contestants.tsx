...
const Contestants: React.FC<{ contestants: ContestantStruct[]; poll: PollStruct }> = ({
  ...
  return (
    <div className="space-y-2">
      <h1 className="text-center text-[48px] font-[600px]">Candidatos</h1>
...
const Contestant: React.FC<{ contestant: ContestantStruct; poll: PollStruct }> = ({
  ...
  const voteContestant = async () => {
    ...
    await toast.promise(
      ...
      {
        pending: 'Aprobar transacción...',
        success: 'Voto emitido exitosamente 👌',
  ...
  return (
    <div className="flex justify-start items-center space-x-2 md:space-x-8 mt-5 md:mx-auto">
      ...
      <div
        className="bg-white bg-opacity-10 backdrop-blur-sm h-[229px] w-[186px] sm:w-[253px] sm:h-fit rounded-[24px]
      ...
      >
        ...
        >
          <div className="w-[32px] h-[32px] rounded-full bg-white bg-opacity-20 backdrop-blur-sm" />
        ...
        >
          {wallet && poll.voters.includes(wallet) ? 'Ya Votaste' : 'Votar'}
        </button>

        <div className="w-[86px] h-[32px] flex items-center justify-center gap-3">
          <div className="w-[32px] h-[32px] rounded-[9px] py-[8px] px-[9px] bg-blue-500 bg-opacity-30">
            <BiUpvote size={20} className="text-[#1B5CFE]" />
          </div>
          <p className="text-[14px] font-[600px]">{contestant.votes} {contestant.votes === 1 ? 'voto' : 'votos'}</p>
        </div>
      </div>
    </div>
  )
